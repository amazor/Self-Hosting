...
def _ensure_alertmanager_config(env: dict[str, str], config_base: Path, tracker: StepTracker) -> None:
    ...
    topic = env.get("NTFY_ALERT_TOPIC", "homelab-alerts").strip() or "homelab-alerts"
...
def _copy_alerting_files(config_base: Path, tracker: StepTracker) -> None:
    """Copy static alerting sources (rules, ntfy templates, Grafana
    provisioning) verbatim into their runtime locations — same pattern as
    the dashboards/*.json copy below. Edit the sources under
    docker_compose/monitoring/alerting/, not the copies under config/.
    """
...
def bootstrap_steps(
    ...
    return [
        ...
        ("Syncing alerting config (rules, ntfy templates)", lambda t: _copy_alerting_files(config_base, t)),
    ]
