...
# TODO(NFS): If monitoring ever needs NFS mounts (e.g. for long-term metric
#   storage on the NAS), add a _reexec_as_root() call at the top of main() —
#   same pattern as docker_compose/media/bootstrap.py. When doing so, ensure
#   setup_observability_config() reads PUID/PGID from env (already fixed in
#   homelab_common.py) and not os.getuid(), or alloy/data will be chowned to
#   root and Alloy will fail to start.
...
# bootstrap writes per-job file_sd files under targets/.
#
...
scrape_configs:
  ...
  # --- Blackbox probe targets (HTTP, TCP, ICMP, DNS) ---
  # Targets come from file_sd; edit blackbox-targets.json to add probes.
  # Each target entry needs a "module" label matching a blackbox.yml module.
  # Example target entry:
  #   {{"targets": ["https://grafana.example.com"], "labels": {{"module": "http_2xx", "name": "grafana"}}}}
  - job_name: "blackbox"
    metrics_path: /probe
    file_sd_configs:
...
# Each module defines a probe type Prometheus can request.
...
def generate_scrape_targets(config_base: Path, env: dict[str, str]) -> None:
    """Parse SCRAPE_TARGETS from .env and write per-job Prometheus file_sd JSON.
    ...
    ne_entries: list[dict] = []
    ca_entries: list[dict] = []
    if raw:
    ...
    if not bb_file.is_file():
        ...
        log.info(
            f"Created empty blackbox targets file: {bb_file}\n"
            "  Add probe targets in this format:\n"
            '  [{"targets": ["https://example.com"], '
            '"labels": {"module": "http_2xx", "name": "example"}}]'
        )

    # Clean up legacy combined file to avoid confusion.
    legacy = targets_dir / "scrape-targets.json"
    if legacy.is_file():
        legacy.unlink()
        log.info(f"Removed legacy combined targets file: {legacy}")

    count = len(ne_entries)
    if count:
...
def print_summary(config_base: Path, env: dict[str, str]) -> None:
    ...
    log.info("  Blackbox Exp:  http://localhost:9115 (probe endpoint)")
...
def main(argv: list[str] | None = None) -> None:
    ...
    ensure_blackbox_config(config_base)
