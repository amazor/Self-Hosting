...
**UI config** is required after deploy: proxy routes, *arr settings, quality profiles, etc. **Exception: core stack SSO** — if `AUTHENTIK_BOOTSTRAP_TOKEN` is set in `.env`, deploy automatically applies the Authentik blueprint via API after starting core (creates proxy providers, applications, and outpost assignment from `:sso` entries in `CADDY_EXTRA_SERVICES`; no manual GUI setup needed). See [Chapter 3A — Authentik](docs/Chapter3a-core-stack.md#authentik) for details. For other UI config, see the **specific docs** for what to do in each app’s UI ([Chapter 3A](docs/Chapter3a-core-stack.md) for core, [Chapter 3c](docs/Chapter3c-media-stack.md) for media).
