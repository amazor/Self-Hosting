...
3. Optionally configure NFS mount (interactive runs only) — also installs a bounded (5 min) ping-retry wait for the NAS host (`wait-for-nas@<host>.service`), ordered before both the mount unit and `docker.service`. Protects against a reboot racing dockerd against a slow NFS mount *and* a full power-outage recovery where the NAS itself is still booting — either way, a `restart: unless-stopped` container no longer gets stuck bind-mounted to an empty pre-mount directory. Never hangs boot indefinitely: it gives up and proceeds after the timeout.
   ...
   > **Why the RW media mount is `hard`, not `soft`.** `ensure_nfs_mount` writes `hard` for read-write mounts (the media root here; Immich's upload root on `accelerated`) and `soft` only for read-only mounts (Plex's library view). A `soft` mount returns an I/O error when an op exceeds `timeo × retrans` — fine for a read-only consumer, but on a RW mount that qBittorrent/SABnzbd/*arr hammer it's a data-safety hazard: a write that times out mid-flight can leave silently partial data, and a slow large-file delete can return EIO *even though it succeeded on the NAS* (observed on a 45 GB file — the *arr treats the EIO as failure and orphans the file, and Plex then shows a stale/duplicate entry). `hard` waits for the op to complete instead of guessing. The usual `hard` objection — hanging forever if the NAS vanishes — doesn't apply here: `nofail` + `_netdev` + the `wait-for-nas` ordering keep *boot* bounded, and runtime NAS loss on this fleet is a whole-fleet power event, not a NAS-only blip. Plex's mount deliberately stays `soft` so a scan-time blip returns EIO (which Plex won't false-trash on) rather than blocking the scanner.
...
**Partly automatable:** the initial *connections* (Sonarr/Radarr/qBittorrent) have no public API in Cleanuparr and still need the one-time UI paste-in below. Everything after that — enabling Download Cleaner and creating its seeding rule — **is** API-driven (Cleanuparr exposes a real REST API once a local account exists, authenticated via `X-Api-Key`) and `setup_media_apps.py` will do it for you if you set `CLEANUPARR_API_KEY` in `.env` (grab it from Account Settings > API Key after the one-time setup below).
...
- `movieImportedCategory` / `tvImportedCategory` (set by `_setup_download_client_qbit` in `setup_media_apps.py`) make Sonarr/Radarr move a torrent into a distinct `movies-imported` / `tv-imported` category **the moment — and only the moment — it confirms a successful import**. Per Sonarr/Radarr's own docs, they then leave torrents in that category alone even when `removeCompletedDownloads` would otherwise apply.
